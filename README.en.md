...
- `xdp-api`: API, authentication, configuration center, and search interfaces.
- `xdp-agent`: host-side collector responsible for Syslog UDP listeners and hot-loaded collection configuration.
- `xdp-worker`: consumes raw events from Kafka, executes pipelines, parses fields, and routes events.
- `xdp-writer`: writes processed events to ClickHouse.
- `web/console`: Vue 3 product console.
...
Besides `README.md` and `README.en.md`, additional product requirements, test cases, database design notes, coding standards, and prototype documentation are kept as local internal materials and are not required for the GitHub release package.
